...
## decisions in `FighterSprite`, the way the codebase tests every decision it pulls out
...
func test_the_animation_name_is_posture_then_heading() -> void:
	assert_eq(
		FighterSprite.animation_name(WALK, LEFT), &"walk_left",
		"the name is <posture>_<heading>, matching the sets in wizard_frames.tres"
	)
	assert_eq(
		FighterSprite.animation_name(CAST, UP), &"cast_up",
...
func test_idle_and_walk_are_told_apart_by_name() -> void:
	...
	assert_true(
		FighterSprite.animation_name(IDLE, DOWN) != FighterSprite.animation_name(WALK, DOWN),
...
func test_every_posture_and_heading_has_a_name() -> void:
	...
	for anim in [IDLE, WALK, CAST]:
		for facing in [DOWN, UP, LEFT, RIGHT]:
			seen[FighterSprite.animation_name(anim, facing)] = true
	assert_eq(
		seen.size(), 12,
