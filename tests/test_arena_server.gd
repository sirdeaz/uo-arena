...
# ── Input buffering: one queued entry per physics tick, since #132 ─────────────────
#
# #132: `set_input` used to write straight into `player.input`, so however many
# `submit_input` packets a burst of jitter delivered between two physics steps, only
# the last one ever actually got simulated — the ones before it were silently never
# stepped at all. These pin the fix: a burst still gets each of its entries their own
# tick, in order, and the acked sequence always names whichever entry actually ran.
